...
Recall that to aggregate data by non-standard date/time intervals, such as six months, you can use generate_series() to create bins with lower and upper bounds 
of time, and then summarize observations that fall in each bin.
*/
  --1 --Use generate_series() to create bins of 6 month intervals. Recall that the upper bin values are exclusive, 
...
SELECT lower, 
       ...
       percentile_disc(0.5) WITHIN GROUP (ORDER BY day) 
                                                        AS median
...
Remember: the time to resolve, or completion time, is 
